import {
  LineBreakClasses, RuleResults, Rules, eot, sot,
...
/** @typedef {import('@cto.af/linebreak').BreakRule} BreakRule */
...
function LBspacesBreak(state) {
  if (state.extra.fancy) {
    if (!isFancySpace(state.next.char, state.next.cls)) {
      ...
      return RuleResults.MAY_BREAK;
    }
    return RuleResults.NO_BREAK;
  }
  return RuleResults.PASS;
}
