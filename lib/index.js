...
/**
 * Split by whitespace that contains a newline.
 *
 * @param {string} str
 * @returns {string[]}
 */
function splitNewlines(str) {
  const res = [];
  let prev = 0;
  for (const ws of str.matchAll(/\s+/g)) {
    if (NEWLINE.test(ws[0])) {
      // Real splitter.
      res.push(str.slice(prev, ws.index));
      prev = ws.index + ws[0].length;
    }
  }
...
/**
 * Goal: split by strings that are all-whitespace, which also include at least
 * one newline character.  This turned out to be difficult with a single
 * regexp without causing a ReDoS.  However, if a caller wants to pass in a
 * regexp (as in the previous version of the interface), their code will still
 * work.
 *
 * @type {Splitter}
 */
const DEFAULT_IS_NEWLINE = {
  [Symbol.split]: splitNewlines,
  [Symbol.replace]: replaceNewlines,
};
 ...
 * @prop {number} [firstCol=NaN] If indentFirst is false, how many columns was
 *   the first line already indented?  If NaN, use the indent width, in
 *   display cells.  If indentFirst is true, this is ignored.
 * @prop {string} [hyphen='-'] String to use when long word is split to next
 *   line with LineWrap.OVERFLOW_ANYWHERE.
 * @prop {boolean} [includeANSI=false] If true, include ANSI escape sequences
 *   in the width of the string.  If false, strips ANSI before calculating
 *   width.
 * @prop {number|string} [indent=''] If a string, indent every line with that
 *   string.  If a number, insert that many {@link indentChar}s at the
 ...
 *   the first line as if it was already indented, giving a short first line.
 * @prop {boolean} [isCJK] If specified, override CJK detection by locale.
 * @prop {Splitter} [isNewline=DEFAULT_IS_NEWLINE] Regular expression that finds
 ...
 * @prop {string} [locale] Which locale to use when splitting by graphemes?
 *   Defaults to current locale of system, as calculated by the JS runtime.
...
*   final display, those escapes will be replaced appropriately.  Defaults to
*   an identity transform.
*/

/**
 * @typedef {object} Part
 * @prop {string} str The string for this part.
 * @prop {boolean} last Is this the last part?  If so, it shouldn't get a
 *   newline appended.
 * @prop {number} length Total width, in display cells.
 */
...
export class LineWrap {
  ...
  *#chunks(text) {
   ...
   * but will be indented.  Each line will yield a Part object.
   *
   * @param {string} text The input string
   * @param {number} [firstCol=NaN] If not NaN, use this column as the start
   *   in addition to the firstCol option (if it was non-NaN).
   * @returns {Generator<Part, void, undefined>}
   */
  *parts(text, firstCol = NaN) {
    if (text.length === 0) {
      if (this.#opts.indentFirst) {
        yield {
          str: this.#indent,
          length: this.#indentWidth,
          last: true,
        };
      }
      return;
   ...
   */
  *lines(text, firstCol = NaN) {
    for (const {str} of this.parts(text, firstCol)) {
      yield str;
    }
  }
  ...
  unwrap(text) {
    return text.replaceAll(
      ...
      /** @type {RegExp} */ (this.#opts.isNewline),
      this.#opts.newlineReplacement
    ).trim();
  }
}
